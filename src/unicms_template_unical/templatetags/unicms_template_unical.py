...
def _get_same_level_items(item, language, exclude_item=True):
    ...
    # if parent:
    items = (
        parent.get_childs(lang=language, exclude=item)
        if exclude_item
        else parent.get_childs(lang=language)
    )
    return {
        "parent": parent.localized(lang=language),
        "items": items,
...
def load_same_level_items(context, exclude_item=True):
    ...
    item = NavigationBarItem.objects.filter(
        menu=context["items"][0].menu,
        is_active=True,
...
def load_current_item_from_menu(context):
    ...
    item = (
        NavigationBarItem.objects.filter(
            menu=context["items"][0].menu,
            is_active=True,
            webpath=context["page"].webpath,
            webpath__alias__isnull=True,
    ...
    if item:
        if item.inherited_content:
            item.inherited_content.translate_as(language)
        if item.publication:
            item.publication.translate_as(language)
